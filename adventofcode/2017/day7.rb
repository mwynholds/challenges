...
class Node
  attr_accessor :name, :weight, :children, :payload

  def self.parse(input)
    node = Node.new
    node.name, w, _, c = input.match(/^(\w+) \((\d+)\)( -> (.*))?$/).captures
    node.weight = w.to_i
    node.payload = c.split(',').map(&:strip) if c
    node.children = []
    node
  end
end
...
class Day7
  ...
  def roots(input)
    input = input.strip
    nodes = parse(input).map { |n| [ n.name, n ] }.to_h
    nodes.each { |_, n| build n, nodes }
    nodes.values
  end

  def build(node, nodes)
    return if node.children.length > 0
    return unless node.payload
    node.payload.each do |name|
      child = nodes[name]
      build child, nodes
      node.children << child
      nodes.delete name
...
cntj (57)
EOF
    assert 1, roots(input).length
    assert 'tknk', roots(input)[0].name
  end
