...
class Day4
  ...
  def valid1?(phrase)
    words = phrase.split
    bag = Bag.new words
    words.all? { |w| bag.count(w) == 1 }
  end

  def valid2?(phrase)
    words = phrase.split
    canonical = words.map { |w| w.split('').sort.join }
    bag = Bag.new canonical
    canonical.all? { |c| bag.count(c) == 1 }
  end

  def solve
    phrases = DATA.readlines
    puts phrases.count { |p| valid1? p }
    puts phrases.count { |p| valid2? p }
  end

  def test
    assert true, valid1?('aa bb cc dd ee')
    assert false, valid1?('aa bb cc dd aa')
    assert true, valid1?('aa bb cc dd aaa')
