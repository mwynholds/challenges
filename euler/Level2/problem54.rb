...
class Hand
  ...
  def rank
    r = stflush;  return r if r
    r = fkind;    return r if r
    r = fhouse;   return r if r
    r = flush;    return r if r
    r = straight; return r if r
    r = thkind;   return r if r
    r = tpair;    return r if r
    r = pair;     return r if r
    r = hcard;    return r
  end
  ...
  def ranks
    @cards.map(&:rank).sort_by_rank
  end

  def stflush
    return nil unless suits.uniq.length == 1
    return nil unless ORDER.index(ranks.join(''))
    [ 9 ] + [ ranks[0] ]
  end

  def fkind
    freq = ranks.freq
    return nil unless freq.length == 2 && freq.value?(4) && freq.value?(1)
    [ 8 ] + [ freq.key(4) ]
  end

  def fhouse
    freq = ranks.freq
    return nil unless freq.length == 2 && freq.value?(3) && freq.value?(2)
    [ 7 ] + [ freq.key(3) ]
  end
  ...
  def thkind
    freq = ranks.freq
    return nil unless freq.value?(3)
    [ 4 ] + [ freq.key(3) ]
  end

  def tpair
    freq = ranks.freq
    pairs = freq.select { |k,v| v == 2 }.map { |k,v| k }.sort_by_rank
  ...
  def pair
    freq = ranks.freq
    singles = freq.select { |k,v| v == 1}.map { |k,v| k }.sort_by_rank
    return nil unless freq.value?(2) && singles.length == 3
    [ 2 ] + [ freq.key(2) ] + singles
  end

  def hcard
    freq = ranks.freq
    singles = freq.select { |k,v| v == 1}.map { |k,v| k }.sort_by_rank
    return nil unless singles.length == 5
    [ 1 ] + singles
  end
  ...
  def compare(rank1, rank2)
    i = 0
    while (true) do
      val1 = i == 0 ? rank1[i] : ORDER.reverse.index(rank1[i])
      val2 = i == 0 ? rank2[i] : ORDER.reverse.index(rank2[i])
...
class Problem54
  ...
  def solve
    ...
    DATA.each do |line|
      ...
      player1 = Hand.new cards[0..4]
      player2 = Hand.new cards[5..9]
  ...
  def test
    assert [9, 'A'], Hand.new("AH TH QH KH JH").stflush
    assert [8, '4'], Hand.new("4H 4D KD 4S 4C").fkind
    assert [7, 'T'], Hand.new("TD TH 4H TC 4S").fhouse
    assert [6, 'J', 'T', '6', '5', '2'], Hand.new("6D 5D TD 2D JD").flush
    assert [5, '7', '6', '5', '4', '3'], Hand.new("4H 6D 7D 3C 5S").straight
    assert [4, 'J'], Hand.new("JD 4C JS 9H JH").thkind
    assert [3, '9', '2', 'K'], Hand.new("9H 9D 2C KS 2S").tpair
    assert [2, 'A', 'Q', '9', '3'], Hand.new("AD QH 9D AC 3C").pair
    assert [1, 'K', 'Q', '9', '7', '5'], Hand.new("5D 7H KS 9C QH").hcard
    puts
  end
